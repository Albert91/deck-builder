...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    if (deckCount >= DECK_LIMIT) {
      return new Response(JSON.stringify({ error: `Deck limit reached (${DECK_LIMIT})` }), {
        status: 403,
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    return new Response(JSON.stringify(newDeck), { status: 201, headers: { 'Content-Type': 'application/json' } });
  } catch (error) {
