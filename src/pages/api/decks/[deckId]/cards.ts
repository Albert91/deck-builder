  ...
  try {
    ...
    try {
      ...
      if (error.message === 'User is not the owner of this deck') {
        return new Response(JSON.stringify({ error: 'Forbidden: User is not the owner of this deck' }), {
          status: 403,
          headers: { 'Content-Type': 'application/json' },
        });
      }
...
export const POST: APIRoute = async ({ params, request, locals }) => {
  ...
  try {
    ...
    try {
      ...
      if (error.message === 'User is not the owner of this deck') {
        return new Response(JSON.stringify({ error: 'Forbidden: User is not the owner' }), {
          status: 403,
          headers: { 'Content-Type': 'application/json' },
        });
      }
      ...
      if (error.message === 'Card limit reached (100)') {
        return new Response(JSON.stringify({ error: 'Card limit reached (100)' }), {
          status: 403,
          headers: { 'Content-Type': 'application/json' },
        });
      }
